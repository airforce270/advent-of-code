...
pub fn process(input: &str) -> std::result::Result<String, AocError> {
    ...
    for line in input.lines() {
        ...
        let one_contains_two = pair1.low <= pair2.low && pair1.high >= pair2.high;
        let two_contains_one = pair2.low <= pair1.low && pair2.high >= pair1.high;
...
fn new_pair(s: &str) -> Pair {
    let low: u32 = s.split("-").nth(0).unwrap().parse().unwrap();
    let high: u32 = s.split("-").nth(1).unwrap().parse().unwrap();
    Pair { low, high }
}
